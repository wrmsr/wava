...
import static com.wrmsr.wava.compile.Compilation.newCastLong;
...
public final class BinaryCompilation
{
    private BinaryCompilation()
    {
    }
    ...
    private static final Map<BinaryOp, JBinaryOp> BINARY_OP_MAP = ImmutableMap.<BinaryOp, JBinaryOp>builder()
            ...
            .put(BinaryOp.ShrU, JBinaryOp.RightShiftUnsigned) // FIXME min(c & 0x1FF) ?
    ...
    public static JExpression compileBinary(Binary node, JExpression left, JExpression right)
    {
        switch (node.getOp()) {
            case RotL: {
                switch (node.getType()) {
